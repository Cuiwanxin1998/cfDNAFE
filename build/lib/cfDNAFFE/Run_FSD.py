...
class runFSD(Base):
    def __init__(
        ...
        **kwargs
        ...
        {O}utput:
        outputfile1: sample.FSD.txt                     65bin_ratio(65~69bp,70~74bp,...,396-400bp)
        ...
        for x in self.getInput("bedgzInput"):
            outputfile = os.path.join(self.getOutput("outputdir"), self.getMaxFileNamePrefixV2(x)) + ".FSD.txt"
        ...
        if verbose:
            ...
            self.multiRun(
                args=args, func=calc_FSD, nCore=maxCore(math.ceil(self.getParam("threads") / 4)),
            )
