...
def bam_process(bamInput,
    ...
    bases = ['A', 'C', 'T', 'G']
    End_motif = {}
    End_motif_output = tmp_split[0] + '_EndMotif.txt'
    Breakpoint_motif = {}
    Break_motif_output = tmp_split[0] + '_BreakPointMotif.txt'
    MDS_output = tmp_split[0] + '_MDS.txt'
    # generated bases seq
    ...
    for read1, read2 in read_pair_generator(input_file):
        ...
        # midpoint = round((rend + rstart + 2) / 2)
        # bed file
...
def calc_FSR(bedgzInput, binInput, windows, outputfile):
    ...
    while step < length:
        ...
        continues_bin = num // 50
        last_bin = num % 50
        for i in range(continues_bin):
            tmp_array = np.zeros(3)
            bin_start = start * windows
            bin_end = (start + 50) * windows - 1
            combine_shorts = correct_shorts[step: step + 50]

            combine_intermediates = correct_intermediates[step: step + 50]
            combine_longs = correct_longs[step: step + 50]
            combine_totals = correct_totals[step: step + 50]
        ...
        if last_bin != 0:
            ...
            # continue
            # tmp_array = np.zeros(3)
            # bin_start = start * 100000
            # bin_end = (start + last_bin) * 100000 - 1
            # combine_shorts = correct_shorts[step: step + last_bin]
            # combine_intermediates = correct_intermediates[step: step + last_bin]
            # combine_longs = correct_longs[step: step + last_bin]
            # combine_totals = correct_totals[step: step + last_bin]
            # if np.std(combine_totals) == 0:
            #     step += last_bin
            #     start = 0
            #     temp_str = chrom[step] + '\t' + str(bin_start) + '\t' + str(bin_end) + '\t' + ','.join(
...
def calc_FSC(bedgzInput, binInput, windows, outputfile):
    print("input file:", bedgzInput, binInput)
    inputbed = pysam.Tabixfile(filename=bedgzInput, mode="r")
    bins = pybedtools.BedTool(binInput)
    length = len(bins)
    ...
    while step < length:
        ...
        for i in range(continues_bin):
            ...
            if np.std(combine_totals) == 0:
                step += 50
                start += 50
                tmp_array[0:200] = np.nan
                temp_str = chrom[step] + '\t' + str(bin_start) + '\t' + str(bin_end) + '\t' + '\t'.join(
                map(str, tmp_array)) + '\n'
                FSCfile.write(temp_str)
                continue
            else:
                tmp_array[150:200] = (combine_totals - np.mean(combine_totals)) / np.std(
                    combine_totals)
            if np.std(combine_shorts) == 0:
                tmp_array[0:50] = np.nan
            else:
                tmp_array[0:50] = (combine_shorts - np.mean(combine_shorts)) / np.std(combine_shorts)
            if np.std(combine_intermediates) == 0:
                tmp_array[50:100] = np.nan
            else:
                tmp_array[50:100] = (combine_intermediates - np.mean(combine_intermediates)) / np.std(combine_intermediates)
            if np.std(combine_longs) == 0:
                tmp_array[100:150] = np.nan
            else:
                tmp_array[100:150] = (combine_longs - np.mean(combine_longs)) / np.std(combine_longs)
    ...
    for i in range(len(marks)):
        ...
        try:
            ...
            if total == 0:
                ...
                tmp_array = np.zeros(3)
                tmp_array[0] = Mfragment / total
                tmp_array[1] = Xfragment / total
                tmp_array[2] = Ufragment / total

                res.append(region + '\t' + '\t'.join(map(str, tmp_array)))

    with open(outputFile, 'w') as f:
        f.write('region' + '\t' + 'M' + '\t' + 'X' + '\t' + 'U' + '\n')
        for i in res:
            f.write(i + '\n')
